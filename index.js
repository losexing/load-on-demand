...
const spawn = require('cross-spawn');//跨平台解决节点的衍生和衍生同步
const helper = require('./helper');//帮助类库
const execPath = process.cwd(); // process.cwd() 是当前执行node命令时候的文件夹地址 ——工作目录，保证了文件在不同的目录下执行时，路径始终不变
...
let destPath = (p = '') => path.resolve(execPath, p); // path.resolve([from ...], to) 将参数 to 位置的字符解析到一个绝对路径里。
...
class Main {
    ...
    showAppInfo() {
        const { name = appname(), redux = false, router = false, cssm = false, testing = false, y = false, table = false } = this.argv;
        this.props = { name, redux, cssm, router, testing, table };
